...
mean(F.hat.test[[i]]%*%B[[i]][p,],na.rm=TRUE))^2)
...
(data.test[which(data.test$ID==i),names.Y[p]] - 
mean(data.test[which(data.test$ID==i),names.Y[p]],na.rm=TRUE))^2)
}
...
return(list(MSE.PC.k=MSE.PC.k,MSE.k=MSE.k,beta.i=beta1.i,MSE.ki=MSE.ki,MSE.Mean.ki=MSE.Mean.ki,yhat.ki=yhat.ki,
