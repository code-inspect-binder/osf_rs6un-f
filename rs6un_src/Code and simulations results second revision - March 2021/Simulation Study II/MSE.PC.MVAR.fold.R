...
MSE.PC.MVAR.fold = function(data.list,k,nComp,Model.type){
...
if (Model.type == 3){
beta0.i = as.matrix(fit[[q]][[2]][1] + fit[[q]][[3]])
beta1.i = matrix(rep(fit[[q]][[2]][2:length(fit[[q]][[2]])],
          each=nrow(fit[[q]][[3]])), nrow=nrow(fit[[q]][[3]]), byrow=FALSE)
}
  
if (Model.type == 4){
beta0.i = as.matrix(fit[[q]][[2]][1] + fit[[q]][[3]][,1])
beta1.fixed = matrix(rep(fit[[q]][[2]][2:length(fit[[q]][[2]])],
             each=nrow(fit[[q]][[3]])), nrow=nrow(fit[[q]][[3]]), byrow=FALSE)
beta1.i = as.matrix(beta1.fixed + fit[[q]][[3]][,2:length(fit[[q]][[2]])])
}
...
- as.matrix(data.test.i[,c(names.F.lag[q],names.F.lag[-q])]) %*% as.matrix(beta1.i[i,]))^2

F.hat.test[which(F.hat.test$ID==n.i),names.F[q]] = as.matrix(data.test.i[,c(names.F.lag[q],names.F.lag[-q])]) %*% as.matrix(beta1.i[i,])
}}
