...
F[n.i,] = VAR.sim(B=Psi.i, n=length(n.i), lag=1, include = "none", varcov=Sigma)
}}

# Create the loading matrix
eye = diag(1,Q)
B = sqrt(1-var.E)*eye[unlist(lapply(1:Q, function(q) rep(q,P/Q))),]
...
Error.var = matrix(0,nrow(data),P)
colnames(Error.var) = sprintf("E%d",seq(1:P))

FB.var = matrix(0,nrow(data),P)
colnames(FB.var) = sprintf("FB%d",seq(1:P))
