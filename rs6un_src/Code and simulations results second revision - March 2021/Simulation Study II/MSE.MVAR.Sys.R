...
#################################################################
#################################################################
###     Predictive Accuracy Multilvel VAR(1) (System)         ###
#################################################################
#################################################################
#################################################################
...
MSE.MVAR.Sys = function(data, p, Model.type){
...
MSE.MVAR.Sys = future_lapply(1:p, function(j) 
MSE.MVAR.Model(data,Model.type,y.list[[j]],y.lag.list[[j]],X.lag.list[[j]]),
future.seed = 0xBEEF)

#MSE.MVAR.Sys = lapply(1:p, function(j) 
#MSE.MVAR.Model(data,Model.type,y.list[[j]],y.lag.list[[j]],X.lag.list[[j]]))
