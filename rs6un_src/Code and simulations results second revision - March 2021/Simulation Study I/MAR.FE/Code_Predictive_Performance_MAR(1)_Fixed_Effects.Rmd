---
title: "Simulation results predictive performance multilevel AR(1) model with fixed effects"
...
#save(RM.Anova.MSPE, file = 'Results_Performance_MSPE_MAR_FE.RData')

load(file = 'Results_Performance_MSPE_MAR_FE.RData')
...
```{r}
Boxplot_MSPE_MAR_FE_Time = ggplot(Performance.MSPE.Mean, aes(x=as.factor(Time), y=MSPE, fill = Model)) + 
...
Boxplot_MSPE_MAR_FE_Time

ggsave("Boxplot_MSPE_MAR_FE_Time.png", width = 6, height = 4)
...
```{r}
Boxplot_MSPE_MAR_FE_Persons = ggplot(Performance.MSPE.Mean, aes(x=as.factor(Persons), y=MSPE, fill = Model)) + 
  geom_boxplot() + stat_summary(fun.y=mean, geom="point", shape=4, size=1) + facet_wrap(~Model) + scale_fill_grey() + theme_bw() +
labs(y = "MSPE", x="Number of persons")
Boxplot_MSPE_MAR_FE_Persons

ggsave("Boxplot_MSPE_MAR_FE_Persons.png", width = 6, height = 4)
