---
title: "Simulation results predictive performance person-specific AR(1) model"
...
#save(RM.Anova.MSPE, file = 'Results_Performance_MSPE_AR.RData')

load(file = 'Results_Performance_MSPE_AR.RData')
...
```{r}
Boxplot_MSPE_ARTime = ggplot(Performance.MSPE.Mean, aes(x=as.factor(Time), y=MSPE, fill = Model)) + 
...
ggsave("Boxplot_MSPE_AR_Time.png", width = 6, height = 4)
...
```{r}
Boxplot_MSPE_ARPersons = ggplot(Performance.MSPE.Mean, aes(x=as.factor(Persons), y=MSPE, fill = Model)) + 
  geom_boxplot() + stat_summary(fun.y=mean, geom="point", shape=4, size=1) + facet_wrap(~Model) + scale_fill_grey() + theme_bw() +
labs(y = "MSPE", x="Number of persons")
Boxplot_MSPE_ARPersons

ggsave("Boxplot_MSPE_AR_Persons.png", width = 6, height = 4)
