...
MSE.AR.Sys.fit = function(data,P){
...
Model = as.formula(paste(names.Y[j],"~",names.Y.lag[j],sep=""))
fit = lm(Model,data.i)
...
resid.i[[j]][[i]] = data.i[,names.Y[j]] - Psi.hat.i[[j]][[i]][1]- 
as.matrix(data.i[,c(names.Y.lag[j])]) %*% as.matrix(Psi.hat.i[[j]][[i]][-1])

MSE.i[[j]][[i]] = (data.i[,names.Y[j]] - Psi.hat.i[[j]][[i]][1]- 
as.matrix(data.i[,c(names.Y.lag[j])]) %*% as.matrix(Psi.hat.i[[j]][[i]][-1]))^2
...
MSE.AR = lapply(1:P, function(j) mean(unlist(lapply(n.ID, function(i) mean(MSE.i[[j]][[i]],na.rm=TRUE)))))

# Compute R square
R2.Sys = lapply(1:P, function(j) sum(unlist(MSE.i[[j]]),na.rm=T))
R2.Sys.Mean = lapply(1:P, function(j) sum(unlist(MSE.Mean.i[[j]])))
R2.Sys.AR = lapply(1:P, function(j) 1 - (R2.Sys[[j]]/R2.Sys.Mean[[j]]))

return(list(Psi.hat.i=Psi.hat.i,Psi.CI.hat.i=Psi.CI.hat.i,MSE.AR=MSE.AR,
R2.Sys.AR=R2.Sys.AR,eps.sd.i=eps.sd.i,resid.i=resid.i))}
