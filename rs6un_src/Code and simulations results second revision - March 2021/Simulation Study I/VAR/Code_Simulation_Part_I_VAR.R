...
source(file="Data.PS.Var.R")
source(file="MVAR.Model.R")
source(file="Psi.PS.Matrix.R")
...
#for (n in 1:length(N)){
#for (p in 1:length(P)){
#Psi.list = lapply(1:N[n], function(i) Psi.PS.Matrix(P[p],b.ar.min,b.ar.max,b.cr.min,b.cr.max))
## Save Psi matrix
#save(Psi.list, file = paste("Psi_N_",N[n],"_P_",P[p],".RData",sep = ""))
#}}
...
## Load Psi matrix
#load(file = paste("Psi_N_",N[n],"_P_",P[p],".RData",sep = ""))

## Monte Carlo replicates

#Sim.Var.Data = Data.PS.VAR(N[n],T[t],Psi.list,cor.Sigma)
