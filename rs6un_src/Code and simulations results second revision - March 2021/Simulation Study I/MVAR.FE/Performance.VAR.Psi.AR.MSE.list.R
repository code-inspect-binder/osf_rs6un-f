...
MSE.Psi.list[w,'MSE.Psi'] = mean(unlist(lapply(1:p, function(q) 
  (MSE.Fit.MVAR.PS.Sys[[1]]$Psi.hat[[q]][-1]-Psi[q,q])))^2)
}  

if (m=='MAR.RE'){
load(file = paste("MSE_Fit_MVAR_N_",n,"_T_",t,"_P_",p,"_R_",r,".RData",sep = ""))
MSE.Psi.list[w,'MSE.Psi'] = mean(unlist(lapply(1:p, function(q)
    (MSE.Fit.MVAR.PS.Sys[[2]]$Psi.hat[[q]][-1]-Psi[q,q])))^2)
}

if (m=='MVAR.FE'){
load(file = paste("MSE_Fit_MVAR_N_",n,"_T_",t,"_P_",p,"_R_",r,".RData",sep = ""))
MSE.Psi.list[w,'MSE.Psi'] = mean(unlist(lapply(1:p, function(q)
    (MSE.Fit.MVAR.PS.Sys[[3]]$Psi.hat[[q]][-1][1]-Psi[q,q])))^2)
}
    
if (m=='MVAR.RE'){
load(file = paste("MSE_Fit_MVAR_N_",n,"_T_",t,"_P_",p,"_R_",r,".RData",sep = ""))
MSE.Psi.list[w,'MSE.Psi'] = mean(unlist(lapply(1:p, function(q)
    (MSE.Fit.MVAR.PS.Sys[[4]]$Psi.hat[[q]][-1][1]-Psi[q,q])))^2)
}  
...
c(MSE.Fit.AR.PS.Sys$Psi.hat.i[[q]][[s]][-1]))), ncol=1, byrow=T))-Psi[q,q]))^2)))
}
...
MSE.Psi.list[w,'MSE.Psi'] = mean(unlist(lapply(1:p, function(q)
  ((colMeans(matrix(unlist(lapply(1:n, function(s) 
MSE.Fit.VAR.PS.Sys$Psi.hat.i[[q]][[s]][-1][q])), ncol=1, byrow=T))-Psi[q,q]))^2)))
}}
