...
MSE.MVAR.fit = function(data,P,Model.type,Psi.true.i){
...
for (p in 1:P){
fit = MVAR.Model(data,Model.type,names.Y[p],names.Y.lag[p],names.Y.lag[-p])

beta1.i[[p]] = coef(fit[[2]])[[1]][,2:(P+1)]
}
...
for (p in 1:P){
Psi.hat.i[[i]][p,] = unlist(beta1.i[[p]][i,names.Y.lag]) 
}
