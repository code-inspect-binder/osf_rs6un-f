...
beta1.i = coef(fit[[q]][[2]])[[1]][,2:(length(names.F.lag[-q])+2)]
...
- as.matrix(data.test.i[,c(names.F.lag[q],names.F.lag[-q])]) %*% t(as.matrix(beta1.i[i,])))^2

F.hat.test[which(F.hat.test$ID==n.i),names.F[q]] = as.matrix(data.test.i[,c(names.F.lag[q],names.F.lag[-q])]) %*% t(as.matrix(beta1.i[i,]))
}}
