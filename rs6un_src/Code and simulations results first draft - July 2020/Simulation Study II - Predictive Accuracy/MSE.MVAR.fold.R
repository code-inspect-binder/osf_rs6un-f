...
MSE.MVAR.fold = function(data.list,k,Model.type,y,y.lag,X.lag){
...
beta1.i = coef(fit[[2]])[[1]][,2:(length(X.lag)+2)]
...
- as.matrix(data.test.i[,c(y.lag,X.lag)]) %*% t(as.matrix(beta1.i[i,])))^2
}
